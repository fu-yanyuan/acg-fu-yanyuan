...
int main() {
  ...
  {
     const Eigen::SparseMatrix<double> sparse = matrix_laplacian.transpose()*matrix_laplacian + 2 * penalty_coeff * matrix_penalty.transpose() * matrix_penalty;
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    Eigen::MatrixXd rhs0 = matrix_laplacian.transpose() * matrix_laplacian * vtx_xyz_ref + 2 * penalty_coeff * matrix_penalty.transpose() * matrix_penalty * vtx_xyz_def;// write some code to compute rhs vector
